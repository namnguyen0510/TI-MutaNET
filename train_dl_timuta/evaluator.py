...
def main(embedding,
    ...
    checkpoint_dirc = '{}/result_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}'.format(study_dirc,embedding, seed, bow_threshold, gene_threshold, hid_dim, out_dim, n_hid_layers, dropout, lr, alpha, gamma)
    try:
    ...
    ## AGGREGATE FEATURES
    if use_gene_exp == 'true':
        print('USE MP and mRNA!')
        features = np.concatenate((muta_bert,muta_bow,x), axis=1)
    else:
        print('USE MP ONLY!')
        features = np.concatenate((muta_bert,muta_bow), axis=1)
    AUC = []
    ACC = []
